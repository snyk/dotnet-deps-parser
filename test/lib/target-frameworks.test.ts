...
test('.Net Visual Basic project target framework extracted as expected', async (t) => {
  ...
  t.deepEqual(
    ...
    'targetFramework array is as expected',
  );
});
...
test('.Net project.assets.json multiple target frameworks extracted as expected', async (t) => {
  ...
  t.deepEqual(
    targetFrameworks,
    ['.NETCoreApp,Version=v1.0', '.NETCoreApp,Version=v2.2'],
    'targetFramework array is as expected',
  );
});
...
test('.Net project.assest.json is not valid json', async (t) => {
  try {
    ...
    t.fail(
      targetFrameworks,
      'Should throw an error for failing to extract the target framework',
    );
  } catch (err) {
    t.match(
      err.message,
      'Extracting target framework failed with error',
